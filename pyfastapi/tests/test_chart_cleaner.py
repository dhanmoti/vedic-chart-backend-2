...
def test_clean_text_maps_retrograde_symbol_to_ascii_marker():
    assert ChartCleaner.clean_text("Saturn ℞\n") == "Saturn (Rx)"


def test_format_response_preserves_retrograde_semantics_in_placements_and_charts():
    ...
    assert formatted["placements"]["Raasi-Saturn"] == "Capricorn (Rx)"
    assert formatted["charts"]["D1"][0] == ["Sun", "Saturn (Rx)"]
